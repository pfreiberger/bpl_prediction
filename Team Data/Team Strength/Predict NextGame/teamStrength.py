...
class PoissonDistrib(object):
	...
	"""
	def __init__(self, database,season,xmin,xmax,RANGE):
	...
	def averageGoals(self):
		...
		"""
		self.getScores()
		self.averageHome="{0:.6f}".format((self.FTHG/(self.range*10))) #80 = 10 games * 8 matchday
		self.averageAway="{0:.6f}".format((self.FTAG/(self.range*10)))
		self.beta="{0:.4f}".format((self.FTHG+self.FTAG)/(2*(self.range*10)))
	...
	def appendToList(self,choice,goal,homeAway,team):
		...
		"""
		self.cursor.execute("SELECT SUM("+goal+"), "+homeAway+" FROM championship"+str(self.season)+" WHERE "+homeAway+"=%s  \
								AND MatchDay BETWEEN "+str(self.xmin)+" AND "+str(self.xmax),(team))
	...
	def calculateStrength(self,averageHomeOrAway):
		...
		while (count < len(self.tmp)):
			if (count%2==0): # Its a team
				self.tmp[count+1]=(float(self.tmp[count+1])/(self.range*1.0)) # *1.0 to get our number if a float format
...
if __name__ == '__main__':	
	try:
		...
		season=[1011]
		print("Updating All Beta Variables in the DB...")
		for year in season:
			distrib=PoissonDistrib(database,year,1,8)
